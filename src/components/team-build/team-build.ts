...
export class TeamBuildComponent {
  ...
  private addplayerToAlert(alert: Alert, chosen: any, chosenPosition: string, positionBefore) {
    if(!this.hasRedCard(chosen._id) && !this.hasOrangeCard(chosen._id)) {
      alert.addInput({
        type: 'radio',
  ...
  hasOrangeCard(playerId: string) {
    return _.findIndex(this.sanctions, o => {
      return o.playerId === playerId && o.sanction === StatType.ORANGE_CARD && !o.done;
    }) > -1;
  }
   ...
   */
  getColor(pos: any) {
    if(this.playerPositions[ pos.key ] && this.hasYellowCard(this.playerPositions[ pos.key ]._id)) {
      return 'yellow';
    } else {
      return pos.class;
    }
